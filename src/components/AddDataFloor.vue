<template>
    <div class="container">
      ...
      <div class="title-container">
        ...
        <div class="quantity-input">
          ...
          <input
            v-model="addDataWarehourse.quantityarea"
            type="number"
            min="1"
            class="form-input"
      ...
      <select class="form-select" v-model="currentWarehouse">
  ...
  const AddData = async () => {
    if(!addDataWarehourse.value.name.trim() || addDataWarehourse.value.storage <= 0 || currentWarehouse.value == null){
  ...
  document.body.style.overflow = "hidden";
    ...
    form.append("quantityarea", addDataWarehourse.value.quantityarea)
    form.append("warehouse", currentWarehouse.value)

    if(addDataWarehourse.value.imagearea != null){
        form.append("image", addDataWarehourse.value.image)
    }
    const res = await axios.post(hostName + '/api/Floor/Add', form, getToken())
    if(res.data.success){
  ...
  const FindAllWarehouse = async () => {
    ...
    if(res.data.success){
        ...
        currentWarehouse.value = res.data.content.data[0].id
        Toast.success("Success")
    }
    router.push("")
  }
